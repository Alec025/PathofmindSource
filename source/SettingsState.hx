...
class SettingsState extends FlxState
{
	...
	// StateChanges
	public static var stateChange:Int = 0;
				...
				FlxTween.tween(brain, {y: 100}, 1, {
					...
					onComplete: function(twn:FlxTween)
					{
						switch (stateChange)
						{
							...
							case 1:
								// FlxG.switchState(new VideoSettingState());
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ANY && Options.controllerTrue == true)
		{
			Options.controller();
		}
	}
...
class VideoState extends FlxSubState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (Options.displayModesTMP != Options.displayModes)
			Options.acceptActivation = true;
		if (Options.fpsVisible != Options.fpsVisibleNRM)
				...
				if (FlxG.mouse.justPressed && canClick)
				{
					switch (optSelection)
					{
						case 0:
							...
							{
								Options.fpsVisible = Options.fpsVisibleTMP;
								Options.displayModes = Options.displayModesTMP;
							}
						case 1:
						...
						case 2:
							...
							if (Options.displayModesTMP != Options.displayModes)
							{
								...
								else
								{
									...
									switch (Math.round(Capabilities.screenResolutionX))
									{
										case 1280:
											Options.resolution = 0;
										case 1336:
		...
		{
			case 1280:
				Options.resolutionLimit = 1;
			case 1336:
				Options.resolutionLimit = 2;
			case 1600:
				Options.resolutionLimit = 3;
			case 1920:
				Options.resolutionLimit = 4;
			case 2560:
				Options.resolutionLimit = 5;
			case 3840:
				Options.resolutionLimit = 6;
		}
				...
				if (FlxG.mouse.justPressed && canClick)
				{
					switch (vidSelection)
					{
						...
						case 2:
							if (Options.resolutionAllowed)
							{
								if (Options.resolution != Options.resolutionLimit)
								{
									Options.resolution += 1;
								...
								{
									Options.resolution -= Options.resolutionLimit;
								}
							}
						case 3:
	...
	override public function create()
	{
		...
		pls = new FlxButton(880, 430, "+", function plus()
		{
			if (Options.textSpeedOptions >= 9 && promptAccepted == false)
			{
				...
				PopupMessage.options = ["Proceed", "Cancel"];
				PopupMessage.textforState = "Going Above The Allowed Amount May Break The Game Are you Sure?";
				PopupMessage.textSize = 32 * Options.textSize;
				PopupMessage.textSpeedOutput = "Add";
				PopupMessage.textArea = 530;
				openSubState(new PopupMessage());
			}
